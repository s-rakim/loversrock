...
console.log('\n=== EVERY WIDGET IS GREY LIQUID GLASS ===');
{
  ...
  check('the chosen level is painted on the glance widgets and the summary',
    /GlassStyle\.apply\(context, views, R\.id\.widget_root\)/.test(read('GlanceWidgetProvider.kt'))
    && /GlassStyle\.apply\(context, views, R\.id\.widget_root\)/.test(read('SummaryWidgetProvider.kt')));
